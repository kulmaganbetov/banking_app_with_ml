...
const COLORS = { safe: "#10b981", blocked: "#ef4444", warning: "#f59e0b", ai: "#ef4444", rule: "#f59e0b" };
...
export default function SecurityCenter() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [l, s] = await Promise.all([api.getSecurityLogs(), api.getSecurityStatus()]);
        setLogs(l); setStatus(s);
      } catch { /* ignore */ }
    }
    load();
  }, []);
  ...
  const threatLevelBg: Record<string, string> = {
    LOW: "from-accent/20 to-accent/5", MEDIUM: "from-warning/20 to-warning/5",
    HIGH: "from-orange-500/20 to-orange-500/5", CRITICAL: "from-danger/20 to-danger/5",
  };
  ...
  const categoryData = status ? [
    { name: "AI Behavioral", count: status.blocksByCategory.ai },
    { name: "Rule-Based", count: status.blocksByCategory.rule },
  ] : [];
              ...
              <BarChart data={categoryData}>
                ...
                <Bar dataKey="count" radius={[6, 6, 0, 0]}>
                  {categoryData.map((entry, index) => (
                    <Cell key={index} fill={entry.name === "AI Behavioral" ? COLORS.ai : COLORS.rule} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs text-gray-400 uppercase tracking-wider border-b border-white/10">
                ...
                <th className="text-center py-3 px-2">Reasons</th>
            ...
            <tbody>
              {logs.map((log) => (
                <tr key={log.id} className="border-b border-white/5">
                  <td className="py-3 px-2 text-gray-400">{formatDate(log.timestamp)}</td>
                  ...
                  <td className="py-3 px-2 text-center">
                    <div className="flex items-center justify-center gap-1">
                      ...
                      {log.blockReasons.length === 0 && <span className="text-xs text-gray-600">—</span>}
                    </div>
                  </td>
                  <td className={`py-3 px-2 text-right font-mono ${
                    log.riskScore > 0.55 ? "text-danger" : log.riskScore > 0.3 ? "text-warning" : "text-accent"
                  }`}>{(log.riskScore * 100).toFixed(0)}%</td>
                  <td className="py-3 px-2 text-center">
                    <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${
              ...
              {logs.length === 0 && (
                <tr><td colSpan={6} className="py-8 text-center text-gray-500">No AI decisions logged yet</td></tr>
              )}
            </tbody>
          </table>
        </div>
